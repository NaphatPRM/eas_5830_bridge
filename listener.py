...
def scan_blocks(chain, start_block, end_block, contract_address, eventfile='deposit_logs.csv'):
    ...
    if end_block - start_block < 30:
        ...
        for evt in events:
            add_val = {
                "chain" : chain, # chain - String (either ‘avax’ or ‘bsc’)
                "token" : evt.args['token'], # token - Address of deposit token (in hex)
                "recipient" : evt.args['recipient'], # recipient - Address of recipient (in hex)
                "amount" : evt.args['amount'], # amount - Number of tokens being transferred
                "transactionHash": evt.transactionHash.hex(), # transactionHash - transaction hash (in hex)
                "address" : evt.address # address - The address of the contract that emitted the event (in hex)
            } 
            data_dicts.append(add_val)
    ...
    with open(eventfile, 'w', newline='') as file:
        fieldnames = ["chain","token","recipient","amount","transactionHash","address"]  # Define column headers
...
if __name__ == "__main__":
    ...
    scan_blocks("bsc", 1, 30, eth_usdc_address)